...
Rattata is a lightweight application framework for single-page JavaScript based applications. The underlying application architecture is based on the MVC (Model, View, Controller) architecture. Rattata really only provides basic functionality and is not meant to be a widget library. The Rattata API arised from building an app with it first and deriving the syntax from it, instead of first developing Rattata.


## Dependencies ##

Rattata depends on the following JavaScript frameworks:
*	stealJS
...
Since stealJS is deeply integrated into Rattata, it is easy let stealJS generate a Rattata based application based on a blueprint. To generate a new app, be sure to have both the 'js' shell application and the 'rattata' framework folder in the directory where you want to generate a new app into. In the terminal, cd to the directory and run
...
2. Configuring your app rattata apps does not need a lot of effort. Open the yourAppName.js file in the newly created folder. Modify the myAppConfig object to fit your needs: specify which files you want to depend on and other options (see 'App configuration').
...
Coding with rattata means defining the global *app* object and executing one of the controllers to start the app.
...
**Note: Observed models are experimental. Rattata relies on beautiful syntax and one requirement was to automatically observe manually added properties and monitor every single model data modification painlessly.  Since the global definition of Setter and Getter in JavaScript still is not standardised, Rattata has to work around with existing methods like Object.defineProperty(). Every top-level AJAX result property will be observed. No recursive tree handling here!**
...
Since you also should handle user interaction in the controller, Rattata provides an easy way to handle UI events. Say you have an HTML element in your app that has been marked as UI element using the class prefix 'ui':
...
In order to define a new view, simply create an HTML file in the ´/resources/views´ folder. Since Rattata used jQote2 as templating engine, you can embed pure JavaScript logic into the template to render JavaScript data. Be sure to have added the view file in your myAppConfig:
...
If the embedded plugins follow the conventions of building rattata plugins and the name of the embedded plugin is ´embeddedPlugin´, the extended functionality is available via the
...
**Note: Be sure to have some set-up automation running before a controller is run by rounting.**
